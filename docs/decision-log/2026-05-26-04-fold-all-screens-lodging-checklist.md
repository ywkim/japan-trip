...
- 그 후 예약이 누적되며 `data/booking-checklist.json`의 `reference`(예약번호)·`action`(권장)이 길어졌다. 특히 사이호지(PR #51) 예약번호는 "Trip.com ① 1400827143416024 (성인2, ₩94,108) · ② 1400827143410570 (성인2) · 6/1 10:30 · LEE/SOYEON · 조건부 취소(건당 수수료 ₩18,822)"로, `checklist_card`의 우측 정렬 셀(`.row .v`, `text-align:right`)에서 모바일 폭을 넘쳐 레이아웃을 깨뜨렸다.
