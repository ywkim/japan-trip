...
- 그 후 예약이 누적되며 `data/booking-checklist.json`의 `reference`(예약번호)·`action`(권장)이 길어졌다. 특히 사이호지(PR #51) 예약번호 등이 길어져 `checklist_card`의 우측 정렬 셀(`.row .v`, `text-align:right`)에서 모바일 폭을 넘쳐 레이아웃을 깨뜨렸다.
