...
- CI(`.github/workflows/validate.yml`): `actions/setup-python` + `pip install` → `astral-sh/setup-uv@v6`(`version: 0.8.17`로 lock 생성 uv와 고정, `python-version: 3.11`, `enable-cache: true`) + `uv sync --locked` + 각 스텝 `uv run python ...`. `--locked`로 lock↔pyproject 불일치 시 실패(재현성 가드). (최초 `@v8` 태그가 미존재(404)라 CI가 액션 해석 단계에서 실패 → 실재 메이저 `@v6`로 정정, uv 버전도 핀.)
...
- [ ] CI에서 `astral-sh/setup-uv@v6`(uv 0.8.17) + `uv sync --locked` 그린 확인
