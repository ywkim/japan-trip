...
- `sw.js`·`manifest.json`·`assets/icon.svg` — **오프라인(PWA) 산출물**. 서비스 워커가 전 페이지·로컬 자산을 사전 캐시해 **비행기 모드에서도 모든 페이지가 다시 열린다**(HTTPS 한 번 방문 후). PWA 매니페스트로 홈 화면 추가(앱 모드) 가능. 외부 사진(장소·블로그)은 온라인에서 한 번 보면 캐시됨. 근거: `docs/decision-log/2026-05-31-offline-service-worker.md`
...
assets/      # og-*.svg (OG 카드 6장) + icon.svg (PWA 앱 아이콘) + lodging/ (숙소 사진, 커밋) — og-*.svg·icon.svg는 build_index.py 산출물(gitignore)
