...
### 5. 보조 데이터 활용
- `data/weather.json` — 후보지 × 시기 기후 데이터 (JMA 평년값) + `tsuyu_normals`(긴키 매우입·매우명 평년 + 최근 7년 실적) + `cities.kyoto.sub_monthly_precip`(순계열 강수)·`trip_window_daily_precip`(5/31~6/3 일별 평년)
- `docs/weather.md` — 시기별 쾌적도 순위, `seasonality`/`physical_burden` 점수 제안. §5에 교토 5/31~6/3 장마 정량 분석 (평년 입림 6/6, 여행 4일 합계 ~16mm, 조기 입림 확률 ~14%)
