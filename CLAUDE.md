...
- **실행 단일 출처(정본)** — 본 레포의 현재 1차 데이터:
  ...
  - `data/breakfast.json` — 숙소 인근 조식 옵션 단일 출처 (아침 3회·숙소별 가게표·아침별 권장·출처). `build_index.py`가 `viz/breakfast.html`로 렌더. 가게명은 `map_query`(없으면 가게명+숙소 `map_area`)로 구글 지도 검색 링크(`.map-link`)로 렌더 — 모바일 탭 시 지도 앱이 열린다(좌표 추정 금지, 검색 질의만 생성). 사람용 사본은 `docs/breakfast-near-lodging.md`
