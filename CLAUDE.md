...
│   └── place-images/            # 자가호스팅한 외부 일정 이미지 (커밋 — fetch_assets.py 다운로드, 서비스 워커 사전 캐시 대상)
├── data/
...
│   ├── local-image-map.json   # 단일 출처 (외부 이미지 url → 자가호스팅 로컬 경로. fetch_assets.py 생성, build_index.py가 빌드 시 치환)
...
│   ├── build_index.py   # index.html + viz/*.html(26개) + assets/og-*.svg(6장) + 오프라인 산출물(sw.js·manifest.json·assets/icon.svg) 빌드. 포함: index + itinerary·itinerary-table·checklist·lodging·archive·breakfast(7개 UI) + DOC_PAGES 22개 마크다운 렌더(report·research·transit-pass 등) + saihoji 커스텀 빌더(1개) + decision-log 인덱스(1개, 총 31). 공통 토큰 주입, 산출물은 gitignore — Vercel·CI·로컬에서 빌드
...
- **현재 22 DOC_PAGES 문서 렌더 페이지** + **1개 커스텀 빌더 페이지** + **1개 인덱스** = 24 페이지:
  - DOC_PAGES 렌더(22개):
    ...
    - 후기·안내(4): `viz/kaneyo-review.html`(카네요 방문기 번역 — 5/31 저녁 blog_reviews)·`viz/shinkyogoku-review.html`(신쿄고쿠 안내 번역 — 5/31 20:00 blog_reviews)·`viz/excafe-review.html`(eX cafe·아라시야마 카페 비교 — 6/1 09:30 Tabelog)·`viz/manzaratei-review.html`(만자라테이 — 6/1 18:30 Tabelog·ぐるなび)
...
4. `python scripts/validate.py` (H 통과) + `python scripts/build_index.py --check` (로컬 재현성·drift 없음) 확인. 산출물은 gitignore이므로 커밋하지 않는다.
